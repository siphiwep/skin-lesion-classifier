...
class ModelUtils():
    ...
    def get_train_data(self, name=FOLDER, folder='../data/', resize=None):
        ...
        if FOLDER == '2017':
            ...
            self.x, self.valX, self.y, self.valY = train_test_split(self.x, self.y, test_size=self.validation, random_state=1000)
            self.x, self.testX, self.y, self.testY = train_test_split(self.x, self.y, test_size=self.test_split, random_state=1000)
            print("Training on {0}".format(len(self.x)))
    ...
    def train(self, model, name=None):
        ...
        aug = tf.keras.preprocessing.image.ImageDataGenerator(
            # preprocessing_fun,ction=tf.keras.applications.ResNet50.preprocess_input,
            rotation_range=90, 
			zoom_range=0.15,
        ...
        self.x = np.concatenate((self.x, self.valX, self.testX), axis=0)
        self.y = np.concatenate((self.y, self.valY, self.testY), axis=0)
        self.x, self.valX, self.y, self.valY = train_test_split(self.x, self.y, test_size=self.validation, random_state=1000)
        self.x, self.testX, self.y, self.testY = train_test_split(self.x, self.y, test_size=self.test_split, random_state=1000)
        print("Training on {0}".format(len(self.x)))
        print("Validating on {0}".format(len(self.valX)))
        ...
        if(os.path.exists('../models/'+self.model.name+FOLDER+name+'.h5')):
            ...
            self.history = self.model.fit(aug.flow(self.x,self.y, batch_size=self.batch_size, shuffle=True, seed=1000),
    ...
    def optimizer(self):
        return tf.keras.optimizers.SGD(lr=0.001, momentum=0.9, decay=0.0005)
        # return Adam(lr=0.0001)

    def confusion_matrix(self, name=None):
        predictions = self.model.predict(self.valX)
        labels = list(set(get_labels(self.valY, dataset='2017'))) 
        print(labels)
        # target_names = ["akiec", "bcc", "bkl", "df", "mel", "nv", "vasc"]
        target_names = ["melanoma", "nevus", "seborrheic_keratosis"]
        ...
        print(classification_report(get_labels(self.valY, dataset='2017'), get_labels(predictions, dataset='2017')))
        cm = confusion_matrix(get_labels(self.valY, dataset='2017'),get_labels(predictions, dataset='2017'))
        ...
        plot_confusion_matrix(cm, target_names, title=name if not None else self.model.name+FOLDER, dataset='2017')
